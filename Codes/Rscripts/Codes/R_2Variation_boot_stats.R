...
for (param in param_list) {
  ...
  if (flag_plot7Locs == TRUE) {
    sink(sprintf("%s/ANOVA_%s%s_%s_nBoot%d_7Loc.txt", nameFolder_Output_variation, str_SF, str_n9, param, nBoot))
  } else {
    sink(sprintf("%s/ANOVA_%s%s_%s_nBoot%d.txt", nameFolder_Output_variation, str_SF, str_n9, param, nBoot))
  }
