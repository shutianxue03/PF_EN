...
template_L = CreateGabor(sz_px, env_px, ORI_deg_all(1), SF_px, template_phase, 2*template_cst);
template_R = CreateGabor(sz_px, env_px, ORI_deg_all(2), SF_px, template_phase, 2*template_cst);
            ...
            for iSignalCst = 1:nSignalCst
                ...
                if flag_simPMF
                    ...
                    for iTrial = 1:nTrials
                        % Extract the correct orientation
%                         ORI = ORI_allTrials(iTrial);
                        ...
                        switch iTvCModel
                            ...
                            case 2
                                ...
                                Nadd=0;
                                Gain=2;
                                paramsPTM_true = [Nmul, Gamma, Nadd, Gain];
                                
                                internalVar_matched = (internalVar*Gain)^Gamma;
                                noise_mul = randn*internalVar_matched*Nmul;
                                noise_add = randn*Nadd;
...
%% Plot each param as a function of error type, SF and perf level
colors_allPerf = {'r', 'b', 'g'};

figure('Position', [0 0 2e3 2e3]), hold on
for iParam=1:nLF_LAM
    subplot(nLF_LAM, 1, iParam), hold on, grid on
    x=1;
    x_ticklabels = cell(nSF*nPerf*nErrorTypes, 1);
    for iSF = 1:nSF
        SF = SF_all(iSF);
        ...
        for iPerf = 1:nPerf
            ...
            for iErrorType = iErrorType_all
                ...
                % Extract param
                switch iParam
                    case 1, param = estP_all{iSF, iPerf, iErrorType}(1); % slope
                    case 2, param = sqrt(estP_all{iSF, iPerf, iErrorType}(2)); % Neq in log cst
                end
                
                plot(x, param, markers_allSF{iSF}, 'color', colors_allPerf{iPerf}, 'MarkerSize', 10, 'LineWidth', 2)
                x_ticklabels{x} = sprintf('%s-%d%%-SF%d', ErrorTypes_all{iErrorType}, perfThresh, SF);
                x=x+1;
            end % iErrorType
        end % iPerf
    end % iSF
    xticks(1:x)
    xticklabels(x_ticklabels), xtickangle(30)
    title(namesLF_LAM_beforeConversion{iParam})
end % iParam

%%
% Initialize variables to store data for ANOVA
anova_data = []; % Data values
anova_sf = [];   % Spatial frequency factor
anova_perf = []; % Performance level factor
anova_error = []; % Error type factor

% Loop through all conditions and extract parameters
for iSF = 1:nSF
    for iPerf = 1:nPerf
        for iErrorType = iErrorType_all
            
            % Extract data for each parameter (adjust for your analysis)
            param_slope = estP_all{iSF, iPerf, iErrorType}(1); % Slope
            param_NeqLog = sqrt(estP_all{iSF, iPerf, iErrorType}(2)); % Neq
            
            % Append data for both slope and Neq (if both are analyzed)
            %             anova_data = [anova_data; param_slope]; % Add slope data
            % Uncomment below if analyzing Neq
            anova_data = [anova_data; param_NeqLog];
            
            % Create factors for each entry
            anova_sf = [anova_sf; iSF]; % Spatial frequency
            anova_perf = [anova_perf; iPerf]; % Performance level
            anova_error = [anova_error; iErrorType]; % Error type
        end
    end
end

% Combine factors into a table for ANOVA
tbl = table(anova_data, anova_sf, anova_perf, anova_error, 'VariableNames', {'Data', 'SF', 'PerfLevel', 'ErrorType'});

% Conduct three-way ANOVA
[p, tbl_stats, stats] = anovan(tbl.Data, {tbl.SF}, 'model', 'full', 'varnames', {'SF'});
[p, tbl_stats, stats] = anovan(tbl.Data, {tbl.PerfLevel}, 'model', 'full', 'varnames', {'Perf Level'});
[p, tbl_stats, stats] = anovan(tbl.Data, {tbl.ErrorType}, 'model', 'full', 'varnames', {'Error Type'});

[p, tbl_stats, stats] = anovan(tbl.Data, {tbl.SF, tbl.ErrorType}, 'model', 'full', 'varnames', {'SF', 'Error Type'});
[p, tbl_stats, stats] = anovan(tbl.Data, {tbl.SF, tbl.PerfLevel}, 'model', 'full', 'varnames', {'SF', 'Perf Level'});
[p, tbl_stats, stats] = anovan(tbl.Data, {tbl.PerfLevel, tbl.ErrorType}, 'model', 'full', 'varnames', {'Perf Level', 'Error Type'});


% Post-hoc tests if necessary
% (e.g., Tukey's HSD or pairwise comparisons)
multcompare(stats, 'Dimension', 1); % Post-hoc for SF
multcompare(stats, 'Dimension', 2); % Post-hoc for Perf Level
multcompare(stats, 'Dimension', 3); % Post-hoc for Error Type
