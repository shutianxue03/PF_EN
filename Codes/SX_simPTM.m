...
%------The Play Zone ------
Gamma0 = 1; Gamma1 = 4; Gamma_null = 1;
Nadd0 = -4; Nadd1 = -2; Nadd_null = -4;
Gain0 = 2; Gain1 = 3; Gain_null = 2;
%--------------------------

params_ref = [Nmul0, Gamma0, Nadd0, Gain0];
params_change = [Nmul1, Gamma1, Nadd1, Gain1];
params_null = [Nmul_null, Gamma_null, Nadd_null, Gain_null];
colors_allLines = [0, 0, 1; 1, 0, 0];  % blue for ref, red for change, gray for turned off
...
noiseSD_full = 10.^linspace(-1.5, log10(.44) , 5);  % Uniform scaling for fitting
...
cst_ln_min = 3 / 100;  % Minimum threshold (natural log)
cst_ln_max = 50/100;        % Maximum threshold (natural log)
...
namesLF = namesLF_PTM;  % Names of parameters
indParamExist = [1, 1, 1, 1];  % Indices for active parameters
...
% Plotting
figure('Position', [0, 0, 1.5e3, 500]);  % Large figure size

iplots = [0, 4,3,2];
for iParam = 2:4
    % Create subplot for each parameter
    subplot(1, 3, iplots(iParam)-1); hold on%; grid on;
    ...
    for iPerf = 1:nPerf
        ...
        % Reference
        %------------------------------------------------------------------------%
        TvN_pred_ref = fxn_PTM(indParamExist, params_ref, noiseEnergy_intp_true, dprime, SF_fit);
        %------------------------------------------------------------------------%
        ...
        if any(iParam==[1,3])
            str_legends{iline} = sprintf('%s=%.2f (d''=%.1f)', namesLF{iParam}, 10.^params_ref(iParam), dprime);
        else
            str_legends{iline} = sprintf('%s=%.2f (d''=%.1f)', namesLF{iParam}, params_ref(iParam), dprime);
        end
        iline = iline+1;

        % Changed
        params_change_ = params_ref; params_change_(iParam) = params_change(iParam);
        %------------------------------------------------------------------------%
        TvN_pred_change = fxn_PTM(indParamExist, params_change_, noiseEnergy_intp_true, dprime, SF_fit);
        %------------------------------------------------------------------------%
        ...
        if any(iParam==[1,3])
            str_legends{iline} = sprintf('%s=%.2f (d''=%.1f)', namesLF{iParam}, 10.^params_change_(iParam), dprime);
        else
            str_legends{iline} = sprintf('%s=%.2f (d''=%.1f)', namesLF{iParam}, params_change_(iParam), dprime);
        end
        iline = iline+1;

        % turned off
        % params_null_ = params_ref; params_null_(iParam) = params_null(iParam);
        % %------------------------------------------------------------------------%
        % TvN_pred_null = fxn_PTM([1,1,1,1], [.4, 1, -2, 2], noiseEnergy_intp_true, 1.5, SF_fit);
        % TvN_pred_null_ = fxn_PTM([1,0,1,1], [.4,  -2, 2], noiseEnergy_intp_true, 1.5, SF_fit);
        % plot(TvN_pred_null, TvN_pred_null_)
        % plot(log10(sqrt(TvN_pred_null)), log10(sqrt(TvN_pred_null_)));
        % %------------------------------------------------------------------------%
        % TvN_pred_null_log = log10(sqrt(TvN_pred_null));
        % plot(noiseSD_intp_log_true, TvN_pred_null_log, line_style, 'Color', colors_allLines(3, :), 'LineWidth', 1.2);
        % str_legends{iline} = sprintf('%s=%.2f (d''=%.1f)', namesLF{iParam}, params_null_(iParam), dprime);
        % iline = iline+1;
