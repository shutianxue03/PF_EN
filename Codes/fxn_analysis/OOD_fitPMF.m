function [cst_log_unik, nCorr, nData, pC, estP_allB, LL_allB, R2_weighted_allB, ...
    converged_allB, pC_pred_allB, thresh_log_allB] = OOD_fitPMF(flag_estimateThresh, ccc_full, fit, thresh_log_stair)
...
LL_allB = nan(nBoot, nModels);
converged_allB = LL_allB;
R2_weighted_allB = LL_allB;
pC_pred_allB = nan(nBoot, nModels, length(curveX_log));
thresh_log_allB = nan(nBoot, nModels, nPerf);
...
%     estP = fmincon(fxn_getnLL, PMF_param0, [], [], [], [], PMF_lb, PMF_ub, [], fit.PMF_options);
    ...
    % Parametric bootstrap (make a copy of this scirpt and wirte a new one without nBoot)
    estP_allB_perM = nan(nBoot, fit.nParams);
    LL_allB_perM = nan(nBoot, 1);
    ...
    [~, estP_allB_perM, LL_allB_perM, converged_allB_perM] = ...
        ...
        estP, fit.paramsFree, fit.nBoot, fit.PF, ...
        'AlphaLimits', alphaLimits, ...
        'LapseLimits', fit.lapseLimits, ...
        'GuessLimits', fit.guessLimits);
