...
if flag_logCST
    ...
    cst_all = log10(cst_all); % do NOT move upward, as it has to be rounded first
...
for icst_unik = 1:length(cst_unik)
    indTrial = (iLoc_all==iLoc_tgt) & (iNoise_all==iNoise) & (cst_all>cst_unik(icst_unik)-0.001 & cst_all<=cst_unik(icst_unik)+0.001);
