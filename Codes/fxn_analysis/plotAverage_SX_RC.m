...
if flag_plot==1
    ...
    for iNoise = 1:nNoise
        ...
        if nNoise>2
            ...
            sgtitle(sprintf('%s (SF=%d, N=%.0f%%) - %s [%s]', name_avg, SF, noiseLvl_all(iNoise)*100, text_noStair(2:end), folderName_extraAnalysis))
    ...
    if nNoise<=2
        ...
        set(findall(gcf, '-property', 'LineWidth'), 'LineWidth',1.5)
        sgtitle(sprintf('%s (SF=%d) - %s [%s]', name_avg, SF, text_noStair(2:end), folderName_extraAnalysis))
        ...
        for iNoise = 1:nNoise
            for iLoc = iLoc_tgt_all
                ...
                title(sprintf('%s (Loc %d, N=%.0f%%) - %s [%s]', ...
                    name_avg, iLoc, noiseLvl_all(iNoise)*100, text_noStair(2:end), folderName_extraAnalysis))
    ...
    for iNoise=1:nNoise
        ...
        sgtitle(sprintf('%s [SF=%d, Noise=%.0f%%]', name_avg, SF, noiseLvl_all(iNoise)*100))
    ...
    if nNoise==2
        ...
        for iNoise = [1, nNoise]
            ...
            title(sprintf('[%s] SF=%d, Noise=%.0f%% [%s] \nHVA=%s; VMA=%s', ...
                name_avg, SF, 100*noiseLvl_all(iNoise), folderName_extraAnalysis, text_HVA, text_VMA))
    ...
    for flag_plotEnergy = [0,1] % 1=plot energy (i.e., cst^2); 0=plot cst
        if nNoise>1
            ...
            for iperf = 1:nPerf
                ...
                title(sprintf('%s [SF%d] [%s] %d%%', name_avg, SF, folderName_extraAnalysis, perfPSE_all(iperf)))
