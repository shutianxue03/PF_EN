...
constant.screenMode = 2; % 1 = 1280x960;2=800x600;3=400x300
constant.EYETRACK =  input('         >>> Eyetracker on? (1=ON, 0=OFF): '); % 0=NO eyetracking
constant.flagSimulate = input('         >>> Simulate? (1=YES, 0=NO): ');
...
if constant.expMode == 2 % practice
    ...
    constant.ind_Neq = nan;% input('         >>> Enter index of noise in a vector: ');%[1:7]; % these are noise levels just for piloting
    constant.nTrialsPerStair = input('         >>> Number of trials per staircase: '); % default is 15
end
constant.nNoiseSD = length(constant.ind_Neq);

% Location
constant.iLoc_tgt_all =nan;%input('         >>> Enter index of locations: ');%[1:9]; %these are locations just for piloting
