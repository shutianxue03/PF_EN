...
run.iNoise = iNoise;
flag_updateStair = 0;
if constant.expMode==1, flag_updateStair = 1; end
if constant.expMode==5
    switch run.dataMode
        case 1 % new noise SD
            params.UD = params.UD1_newNoiseSD;
            iNoise = find(params.noiseSD_new_all == run.extNoiseLvl);
            flag_updateStair = 1;
        case 3 % new loc
            params.UD = params.UD3_newLoc;
            iNoise = find(params.noiseSD_full == run.extNoiseLvl);
            flag_updateStair = 1;
    end
end

% get Gabor contrast
if find(constant.expMode==[1,2])
...
% quickPlot_stim % SX
%%%
% stim_tgt = limit0to1(.5 + gabor*.5 + filtered_noise).*stim.mask + noise.ratio_base*(1-stim.mask);
% stim_noise = limit0to1(.5 + filtered_noise).*stim.mask + noise.ratio_base*(1 - stim.mask);
limit0to1 = @(x) min(max(x,0),1);
for iframe = nframes_beforeGabor + (1:nframes_Gabor) %Iterate through the number of frames
    % noiseImg(:,:,iframe,run.targetLoc) = (noiseImg(:,:,fr,run.targetLoc)+gabor); % AB
    noiseImg_(:,:, iframe, run.targetLoc) = limit0to1(.5 + gabor*.5 + noiseImg(:,:,iframe, run.targetLoc)).*mask + .5*(1-mask);
end
% stimImg = min(max(noiseImg + visual.bgColor, 0),255); % AB, moved up here by SX
stimImg = noiseImg_; % SX, divide the pixels by 255 to avoid saturation is using a [0,1] scale
quickPlot_stim % SX
%%%
