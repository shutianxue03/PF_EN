...
for i=1:length(p.modelOption)
    if isempty(p.modelOption{i})
        ...
        switch p.modelOption{i}
            case 'Am'
                Am(1:5,2) = params(count:count+4);
                count = count+5;
                Am(1:5,3) = params(count:count+4);
                count = count+5;
            case 'Ae'
                Ae(1:5,2) = params(count:count+4);
                count = count+5;
                Ae(1:5,3) = params(count:count+4);
                count = count+5;
            case 'Aa'
                Aa(1:5,2) = params(count:count+4);
                count = count+5;
                Aa(1:5,3) = params(count:count+4);
                count = count+5;
        end
    end
end
...
temp_params(4) = params(8); % [Nm]

for i = 1:nSF
    for j = 1:ndifficulty
        for k = 1:ngroup
            curveid = (i-1)*6 + (j-1)*3 + k;
            temp_params(3) = [params([2+i]);];
            temp_params(5:7) = [Am(i,k); Ae(i,k); Aa(i,k)];
