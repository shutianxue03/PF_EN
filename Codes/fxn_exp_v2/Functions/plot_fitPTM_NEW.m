function plot_fitPTM(modelName,paramsfit1_all,paramsfit2_all,paramsfit3_all)
% close all;
% figure('Name',modelName,'Color','white','units','normalized','outerposition',[0 0 1 1],Paper, 'PaperOrientation', 'landscape')% plots all three groups fit by PTM3.m
figure('Name',modelName,'Color','white','units','normalized','outerposition',[0 0 1 1])% plots all three groups fit by PTM3.m
% figure('Name',modelName,'Color','white')

fontSize = 26;
mkSiz    = 12;
% colorCond  = [0   .3  .6; .95 .6 .65; .9 0 0; .75 0 .1];
% colorPatch = [.6 .7  1;  1 .7 .9; .9  .5 .8;  .9 .5 .5];
% colorCond  = [0  .3 .6; .925 .65 .85; .875 .375 .55; .775  0 0];
...
for whichSF = 1:5
    ...
    %     eval(['data_weibull' (SFs{whichSF})]);
    load(['dataNEW_WB' (SFs{whichSF})]);
...
nCol = 6;
    ...
    set(h1,'MarkerSize',mkSiz,'MarkerEdgeColor',colorCond(2,:),'color',colorCond(2,:),'Linewidth',1.5)
    ...
    set(h1,'MarkerSize',mkSiz,'MarkerEdgeColor',colorCond(3,:),'color',colorCond(3,:),'Linewidth',1.5)
    ...
    set(h1,'MarkerSize',mkSiz,'MarkerEdgeColor',colorCond(1,:),'color',colorCond(1,:),'Linewidth',1.5)
    h1=loglog(X_plot,exp(predictedcontrast(paramsfit1,X_plot,d(1))).*100,'-');
    set(h1,'color',colorCond(1,:),'Linewidth',3)
    axis(axismat);
    set(gca,'Layer','top','Linewidth',2,'Box','off','PlotBoxAspectRatio',[.66,1,1],'TickDir','out','TickLength',[1,1]*0.02/max(1,1));
    set(gca,'FontName','Helvetica','FontSize',fontSize);
%     set(gcf,'color','w','Position',[5 5 50 50])
...
saveas(gcf,[pwd '/FIGURES/PTM_allSF_NEW_' modelName '.pdf']);