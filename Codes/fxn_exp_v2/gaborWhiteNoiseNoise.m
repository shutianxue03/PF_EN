function [image] = gaborWhiteNoiseNoise(sizePixels,cyclesPerImage,phase_offset,stim_contr,noise_contr,...
    nMask,mask,cRange,stimTime,ifi,background)
...
frames = round((stimTime/1000)/ifi);   %get the total number of frames 
...
for f = 1:frames                                %Iterate through the number of frames
    image(:,:,f) = (image(:,:,f)+gabor) .* nMask;       %Apply the noise mask to the frame
end
