...
% design.stimDur  = 0.10;  % target duration [s] % for SX_long
...
design.nNoiseSD = length(params.extNoiseLvl); % change to nNoiseSD later
...
constant.nTrialsPerBlock_exp1 = 120; % default:144
constant.nTrialsPerBlock_exp3 = 120; % default: 12
...
for irep = 1:design.nRepeat
        ...
        for iNoise = 1:design.nNoiseSD
            for iStair = 1:design.nStairs
                ...
                for i_nTrials = 1:params.nTrialsPerStair
                    sequenceMatrix_perRep = [sequenceMatrix_perRep; constant.CUE, iLoc, ...
                        params.extNoiseLvl(iNoise), iStair, ORI];
                end
            end
        end
    end
    ...
    % catch trials
    for iLoc = constant.iLoc_tgt_all%1:design.nLoc_tgt
        for iNoise = 1:design.nNoiseSD%1:design.nNoiseSD
...
for iLoc_tgt = constant.iLoc_tgt_all%1:design.nLoc_tgt
    if iLoc_tgt>=6, params.startLvl = params.startLvl_low; % at 8 deg, cst range are higher because threshold are higher
    else, params.startLvl = params.startLvl_high;
    end
    for iNoise = 1:design.nNoiseSD
        if iNoise==1
            if iLoc_tgt == 1 % Fovea, change max to 10%
                startLvl =  log10([1, 10, 1, 10]/100);
            elseif any(iLoc_tgt == [7,9]) % VM at 8, change min to 
                startLvl =  log10([50, 100, 50, 50]/100);
            end
        else
            startLvl = params.startLvl;
        end
        for iStair = 1:design.nStairs
            params.UD{iLoc_tgt, iNoise, iStair} = PAL_AMUD_setupUD('up',1,'down',params.stairRule(iStair),...
                'stepSizeUp',params.stairStep_up(1),'stepSizeDown',params.stairStep_down(1),...
                'stopCriterion','trials','stopRule',params.stairStopRule,'startValue',startLvl(iStair),...
...
if constant.expMode==1
    for iLoc_tgt = constant.iLoc_tgt_all%1:design.nLoc_tgt
        if iLoc_tgt>6, params.minVal = log10(.2); params.maxVal = log10(.99); % at 8 deg, cst range are higher because threshold are higher
        else, params.minVal = log10(.02); params.maxVal = log10(.5);
        end
        for iNoise = 1:design.nNoiseSD
            for iStair = design.nStairs + (1:design.nStairsCatch)
