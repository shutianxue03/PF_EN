...
% second version: 5 contrast levels, but no anchoring points at lowest and
% highest cst levels
...
fprintf('PSE loaded\n')
nAnchor = 5;
nNoiseLvL = 1;%length(params.extNoiseLvl);
nLoc = 3;%design_old.nLoc;
if length(PSE_allC)==1, iccc=1;else, iccc = 3;end % make decisions based on all previous trials (1=stair, 2=constim)
iperf=1;
...
flag_quickPlotPMF = 1;
cst_anchor = cell(nLoc, nNoiseLvL);
close all
if flag_quickPlotPMF
    figure('Position', [0 0 nNoiseLvL * 300 nLoc*300])
end
for iLoc_tgt = constant.iLoc_tgt_all
    iiLoc = find(iLoc_tgt == constant.iLoc_tgt_all);
    for iNoise = 1:nNoiseLvL
        anchor_center = log10(PSE_allC{iccc}(iLoc_tgt, iNoise, iperf))-.05; % cst threshold measured by staircase, 70% acc, on log scale
...
%     happyWithAnchor = input('     Happy with the anchor points? (1=YES, 0=NO): ');
