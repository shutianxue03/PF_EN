...
perfThresh_plot = perfThresh_all(iPerf_plot);
...
% if flag_noStair == 2, nModels=1; else, 
    nModels = 4; 
%     end
...
for iModel = 1:nModels
    ...
    [yfit_med, yfit_lb, yfit_ub] = getCI(yfit_allB{iLoc, iNoise, iModel}, 1, 1);
    [thresh_med, thresh_lb, thresh_ub] = getCI(thresh_allB(:, iLoc, iNoise, iModel, iPerf_plot), 1, 1);
    %%%%%%%%%%%%
    ...
    yline(perfThresh_plot/100, '-', 'color', ones(1,3)*.7, 'HandleVisibility', 'off'); % 75%, which should converge with threshold estimated from titration
...
if nNoise>2
    ...
    if nLoc==9, xticks_log = -2:.25:0;
    elseif nLoc==5, xticks_log = -3:.2:0;
    end
else
    xticks_log = -2:.1:0;
...
title(sprintf('L#%d N#%d', iLoc, iNoise))
% iplot = iplot + 1;
